  ...
  const handleFilesSelected = useCallback(async (files: FileList) => {
    ...
    for (let i = 0; i < fileArray.length; i++) {
      ...
      try {
        ...
        toast.success(`Loaded: ${file.name.length > 20 ? file.name.substring(0, 20) + '...' : file.name}`);
      } catch (error) {
      ...
      <div className="container max-w-3xl mx-auto py-3 px-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground font-sans">Audio Mixer</p>
          </div>
        </div>
      </div>
    </header>

    <main className="container max-w-3xl mx-auto py-5 md:py-8 px-4 md:px-6">
      {finalMix && finalBuffer ? <MixOutput audioBlob={finalMix} audioBuffer={finalBuffer} onReset={handleReset} /> : <div className="space-y-6">
        ...
        {clips.length === 0 && !isProcessing && <p className="text-center text-lg font-sans italic text-primary/80 text-warm-glow">
          Curate your dance symphony
        </p>}

        {isProcessing && <div className="flex items-center justify-center gap-2 py-3 text-muted-foreground font-sans text-sm">
        ...
        {clips.length > 0 && <>
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-foreground font-heading">
              {clips.length} {clips.length === 1 ? 'Track' : 'Tracks'}
            </h2><span className="text-xs text-muted-foreground">
          ...
          <button onClick={handleGenerateMix} disabled={isGenerating || clips.length === 0} className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-heading font-medium hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all warm-glow flex items-center justify-center gap-2">
            {isGenerating ? <>
